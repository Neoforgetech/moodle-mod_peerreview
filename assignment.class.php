...
//        echo '<input type="hidden" name="menuindex" value="0" />';//selected menu index
	...
	function mass_grade() {
		...
		*/
        ...
        $criteriaList = $DB->get_records_list('peerreview_criteria','peerreview',array($this->assignment->id),null,'ordernumber');
		if($criteriaList && count($criteriaList)>0) {
            ...
            $query = 'SELECT a.userid, SUM(r.completed) as reviewscomplete, a.timemarked, a.timecreated, a.id, u.firstname, u.lastname '.
                     'FROM '.$CFG->prefix.'assignment_submissions a, '.$CFG->prefix.'peerreview_review r, '.$CFG->prefix.'user u '.
            ...
            if($submissions && count($submissions)>0) {
                ...
                $tableheaders = array(
                                  ...
                                  get_string('status'),
                                  get_string('grade'),
                                  get_string('emailstatus', 'assignment_peerreview')
                                );
                $table->define_headers($tableheaders);
                    ...
                    if (!$grading_info->items[0]->grades[$submissions[$i]->userid]->locked &&
                        ...
                        if($grade!='???') {
                            if($submission = $this->set_grade($submissions[$i]->userid,$grade)) {
                                $status = get_string('gradeset','assignment_peerreview');
                                $gradeToDisplay = $this->display_grade($grade);
                                $email = ($submission->mailed?get_string('emailsent','assignment_peerreview'):get_string('emailnotsent','assignment_peerreview'));
                            }
                            else {
                                $status = get_string('unabletoset','assignment_peerreview');
                                $gradeToDisplay = get_string('unabletowritetodb','assignment_peerreview');
                                $email = get_string('emailnotsent','assignment_peerreview');
                            }
                        }
                        else {
                            $status = get_string('unabletoset','assignment_peerreview');
                            $gradeToDisplay = get_string('moderationrequired','assignment_peerreview');
                            $email = get_string('emailnotsent','assignment_peerreview');
                        }
                    }
                    else {
                        $status = get_string('previouslyset','assignment_peerreview');
                        $gradeToDisplay = get_string('nochange','assignment_peerreview');
               ...
               $OUTPUT->notification(get_string('nocompletesubmissions','peerreview'));
            }
        }
        else {
           $OUTPUT->notification(get_string('nocriteriaset','peerreview'));
        }
        
		$OUTPUT->continue_button($CFG->wwwroot.'/mod/peerreview/submissions.php?id='.$this->cm->id);
        echo '</div>';
	  $OUTPUT->footer();
	}
